...
function DeviceDetailModal({ isOpen, onClose, deviceId, routerId, onOpenWanConfig, onOpenWifiConfig, onOpenRestore }) {
                                    ...
                                    <div className="bg-slate-900/50 p-6 rounded-xl border border-slate-800">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
                                            <div className="flex justify-between py-2 border-b border-slate-800">
                                                <span className="text-sm text-slate-400">PPPoE IP</span>
                                                <span className="text-sm text-primary font-mono font-bold">{fullDevice?._pppoeIp || 'N/A'}</span>
                                            </div>
                                            <div className="flex justify-between py-2 border-b border-slate-800">
                                                <span className="text-sm text-slate-400">Management IP</span>
                                                <span className="text-sm text-white font-mono">{fullDevice?._mgmtIp || 'N/A'}</span>
                                            </div>
                                            <div className="flex justify-between py-2 border-b border-slate-800">
                                                <span className="text-sm text-slate-400">Connection Mode</span>
                                                <span className="text-sm text-white">{fullDevice?._pppoeUser ? 'PPPoE' : 'Static/DHCP'}</span>
                                            </div>
                                            <div className="flex justify-between py-2 border-b border-slate-800">
                                                    ...
                                                    >
                                                        ...
                                                        <td className="px-4 py-3">
                                                            <div className="flex justify-end gap-1">
                                                                ...
                                                                <button
                                                                    onClick={(e) => { e.stopPropagation(); createBackupMutation.mutate({ id: dev._id, name: `Manual Backup ${new Date().toLocaleDateString()}`, routerId: selectedRouterId }); }}
                                                                    className="p-1.5 rounded-md hover:bg-slate-700 text-slate-400 hover:text-amber-400 transition-colors"
                                                                    title="Create Backup"
                                                                >
                                                                    <Database className={clsx("w-3.5 h-3.5", createBackupMutation.isPending && createBackupMutation.variables?.id === dev._id && "animate-pulse")} />
                                                                </button>
                                                                <button
                                                                    onClick={(e) => { e.stopPropagation(); setRestoreDevice({ ...dev, routerId: selectedRouterId }); }}
                                                ...
                                                <div className="flex items-start justify-between">
                                                    ...
                                                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity mr-10">
                                                        ...
                                                        <button
                                                            ...
                                                            title="Create Backup"
                                                        >
                                                            <Database className={clsx("w-4 h-4", createBackupMutation.isPending && createBackupMutation.variables?.id === dev._id && "animate-pulse")} />
                                                        </button>
                                                        <button
                                                            onClick={(e) => { e.stopPropagation(); setRestoreDevice({ ...dev, routerId: selectedRouterId }); }}
                                                            className="p-1.5 rounded-md hover:bg-slate-700 text-slate-400 hover:text-purple-400 transition-colors"
