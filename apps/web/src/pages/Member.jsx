        ...
        <div className="min-h-screen bg-[#020617] text-white flex items-center justify-center px-4">
            <div className="w-full max-w-sm">
                ...
                <div className="text-center mt-6">
                    <a href="/cekstatus" className="text-xs text-slate-500 hover:text-slate-300">Cek status cepat tanpa login →</a>
                </div>
            </div>
        </div>
    );
...
function Dashboard() {
    ...
    if (loading) return <div className="min-h-screen bg-[#020617] flex items-center justify-center text-slate-400"><Loader2 className="w-6 h-6 animate-spin" /></div>;
    if (error) return (
        <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center p-4 text-center">
            ...
            <button onClick={logout} className="mt-4 text-sm text-slate-400 hover:text-white">Kembali ke Login</button>
        </div>
    );
        ...
        <div className="min-h-screen bg-[#020617] text-white">
            <div className="max-w-3xl mx-auto px-4 py-6">
                <header className="flex items-center justify-between mb-6">
                    <div>
                        <div className="text-xs text-slate-500 uppercase tracking-wide">Halo,</div>
                        <h1 className="text-xl font-bold">{c.name}</h1>
                        <div className="text-xs text-slate-400 font-mono">{c.code}</div>
                    </div>
                    <button onClick={logout} className="text-slate-400 hover:text-red-400 text-sm flex items-center gap-1">
                        <LogOut className="w-4 h-4" /> Logout
                    </button>
                </header>

                <section className="space-y-3 mb-8">
                    <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wide flex items-center gap-2"><Wifi className="w-4 h-4" /> Layanan</h2>
                    {profile.subscriptions.length === 0 ? (
                        ...
                        <div key={s.id} className="bg-slate-900/60 border border-slate-800 rounded-lg p-3 sm:p-4">
                            ...
                            {s.nextDueAt && <div className="text-xs text-slate-400 mt-1">Tagihan berikut: {fmtDate(s.nextDueAt)}</div>}
                            {s.expiresAt && <div className="text-xs text-slate-400 mt-1">Berakhir: {fmtDate(s.expiresAt)}</div>}
                        </div>
                    ))}
                </section>
                ...
                <section>
                    ...
                    {invoices.length === 0 ? (
                        <div className="text-sm text-slate-500 bg-slate-900/40 border border-slate-800 rounded-lg p-4">Belum ada tagihan.</div>
                    ) : (
                        <div className="space-y-2">
                            {invoices.map(i => (
                                <div key={i.id} className="bg-slate-900/60 border border-slate-800 rounded-lg p-3 sm:p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                    <div className="min-w-0 flex-1">
                                        ...
                                        <div className="text-xs text-slate-400 mt-0.5">Jatuh tempo: {fmtDate(i.dueAt)}</div>
                                    </div>
                                    {(i.status === 'unpaid' || i.status === 'overdue') && (
                                        <button onClick={() => { setLinkTarget(i); setLinkResult(null); }} className="bg-primary hover:bg-primary/90 text-white text-sm px-3 py-1.5 rounded-lg flex items-center gap-1.5 whitespace-nowrap">
            ...
            {linkTarget && (
                <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => { setLinkTarget(null); setLinkResult(null); }}>
                    <div className="bg-slate-900 border border-slate-700 rounded-xl max-w-md w-full p-5" onClick={e => e.stopPropagation()}>
                        <h3 className="font-semibold text-white mb-1">Bayar {linkTarget.invoiceNumber}</h3>
                        <p className="text-sm text-slate-400 mb-4">{fmtIDR(linkTarget.amount)}</p>
                        {!linkResult ? (
                            <form onSubmit={createLink} className="space-y-3">
                                <label className="block">
                                    <span className="block text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1">Pilih Metode</span>
                                    <select name="gateway" defaultValue="tripay" className="w-full bg-slate-800 border border-slate-700 rounded px-3 py-2 text-white">
                                ...
                                <div className="flex gap-2 justify-end">
                                    <button type="button" onClick={() => setLinkTarget(null)} className="px-3 py-1.5 text-sm text-slate-400 hover:text-white">Batal</button>
                                    <button type="submit" disabled={creating} className="bg-primary hover:bg-primary/90 disabled:opacity-60 text-white text-sm px-4 py-1.5 rounded-lg flex items-center gap-1.5">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex gap-2 justify-end">
                                    <button onClick={() => { navigator.clipboard.writeText(linkResult.paymentUrl); }} className="text-sm text-slate-400 hover:text-white px-3 py-1.5 flex items-center gap-1"><Copy className="w-3.5 h-3.5" /> Copy</button>
                                    <a href={linkResult.paymentUrl} target="_blank" rel="noreferrer" className="bg-primary hover:bg-primary/90 text-white text-sm px-4 py-1.5 rounded-lg flex items-center gap-1.5"><ExternalLink className="w-3.5 h-3.5" /> Bayar Sekarang</a>
                                </div>
                                <p className="text-xs text-slate-500">Status tagihan akan otomatis terupdate setelah pembayaran sukses.</p>
                            </div>
                        )}
                    </div>
                </div>
